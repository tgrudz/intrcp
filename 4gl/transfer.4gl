   ...
   call rcp_tmp_table()
    ...
    menu "Akcja"
      command "Import" "Import danych z RCP"
         ...
         call rcp_imp_kont() 
      command "Kontrola" "Kontrola zapisów RCP - SILP" 
...
function rcp_tmp_table()
   ...
   create temp table tmp_rcp
      (abs_hours      char(4),  --wymiarNieob - ilo¶æ godzin nieobecnosci
   ...
   input by name mr_rcp_adm.server_rcp, 
      ...
      after input
         ...
         if mr_rcp_adm.server_rcp is null then
            error "Pole musi byæ wype³nione"
...
function rcp_imp_kont()
   ...
   if m_import_fl then
      let l_txt = "Importowaæ dane z RCP" 
      let l_kom = "IMPORT   DANYCH"
   else
      let l_txt = "Kontrolawaæ dane z RCP z SILP"
   ...
   if licz_rek = 0 then
      error 'Brak pracowników w bazie!'
      return
   end if
...
define
   ...
   case stat
      when -1
         ...
         return false
      when -2
         call rcp_zapisz_do_rap("UWAGA - niepe³ny ostatni dzieñ absencji !!!")
         return false
      when -3
...
define
   ...
   then
      let m_txt = "B³êdna definicja okresu zliczania absencji !!!"
      return false
   end if
